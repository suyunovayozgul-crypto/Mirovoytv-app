...
// MUHIM (2026-08-26 ANR tuzatish): oldin `resetToken` (retryKey) SafeVlcPlayer
// ustidagi `key` propga qo'shilgan edi — bu tez-tez xato (onError) kelganda
// SafeVlcPlayer'ning O'ZINI ham to'liq unmount/mount qilib yuborardi.
// Unmount paytida quyidagi useEffect ICHIDAGI "background release" logikasi
// ISHLAMAYDI (chunki u faqat source.uri o'zgarganda, ya'ni komponent hali
// TIRIK bo'lganda ishga tushadi) — shu sabab VLCPlayer kutubxonasining O'ZI
// componentWillUnmount'da libvlc_media_player_release() ni to'g'ridan-to'g'ri,
// main thread'da chaqirib yuborardi. Bir nechta xato ketma-ket, tez-tez kelsa
// (masalan noto'g'ri format/kodek bilan kanal), bir necha VLCPlayer bir-birining
// orqasidan zудлик bilan mount/unmount bo'lib, main thread'ni bloklab, ANR
// ("Input dispatching timed out") ga olib kelardi.
//
// Yechim: SafeVlcPlayer'ning o'zini retryKey bilan qayta o'rnatmaymiz (u indi
// barqaror qoladi), faqat ICHKARIDAGI <VLCPlayer>'ni resetToken orqali qayta
// o'rnatamiz — shunda tashqi wrapper effekt HAR DOIM (URL o'zgarganda HAM,
// faqat qayta urinishda HAM) eski instance'ni background'da xavfsiz release
// qila oladi. Qo'shimcha ravishda, komponent butunlay unmount bo'lganda ham
// (masalan foydalanuvchi pleyerdan chiqib ketganda) xuddi shu xavfsiz yo'l
// bilan release qilinadi.
function SafeVlcPlayer({ source, style, resetToken, ...props }: any) {
  const vlcRef = useRef<any>(null);
  const prevKeyRef = useRef<any>(null);

  const releaseOldInstance = useCallback((oldRef: any) => {
    if (!oldRef) return;
  ...
  useEffect(() => {
    // Manba YOKI resetToken (qayta urinish) o'zgarganda — eski playerni
    // background da release qilib, keyingi renderda yangi <VLCPlayer>
    // key orqali qayta o'rnatiladi (pastga qarang).
    const currentKey = `${source?.uri}::${resetToken}`;
    if (prevKeyRef.current !== null && prevKeyRef.current !== currentKey) {
      releaseOldInstance(vlcRef.current);
    }
    prevKeyRef.current = currentKey;
  }, [source?.uri, resetToken, releaseOldInstance]);

  useEffect(() => {
    // Komponent butunlay unmount bo'lganda (masalan pleyerdan chiqilganda)
    // ham xavfsiz, background release yo'lidan foydalanamiz.
    return () => {
      releaseOldInstance(vlcRef.current);
    };
...
function AppInner() {
  ...
  // ── ANR tuzatish (2026-08-26): onError darhol (0ms ichida) qayta urinishga
  // olib kelardi. Ba'zi kanallar formatini ochib bo'lmaganda, ExoPlayer/VLC
  // deyarli bir zumda xato berib turadi — natijada bir necha player instance
  // 100ms ichida ketma-ket yaratilib/o'chirilib, video kodek resurslarini
  // (hw-video-codec) tirillatib, main thread'ni bloklab qo'yardi (ANR:
  // "Input dispatching timed out"). Endi har bir qayta urinishdan oldin
  // kichik "sovutish" pauzasi qo'yamiz va bir vaqtda faqat bitta urinish
  // navbatda turishini ta'minlaymiz.
  const RETRY_BACKOFF_MS = 600;
  const retryPendingRef = useRef(false);
  const retryBackoffTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const attemptRetryOrFail = useCallback(() => {
    ...
    // Allaqachon navbatda turgan qayta urinish bo'lsa — yana bittasini
    // qo'shmaymiz (masalan onError va taymout bir vaqtda kelsa)
    if (retryPendingRef.current) return;
    if (retryCountRef.current < MAX_AUTO_RETRY) {
      retryPendingRef.current = true;
      setPlayerLoading(true);
      setPlayerError(false);
      if (retryBackoffTimeoutRef.current) clearTimeout(retryBackoffTimeoutRef.current);
      retryBackoffTimeoutRef.current = setTimeout(() => {
      ...
      // (Bu yerda ham backoff bilan — ExoPlayer'dan VLC'ga o'tish o'zi
      // eski instance'ni yo'q qilib, yangi kodek instance ochadi, shuning
      // uchun darhol emas, biroz kutib bajaramiz.)
      retryPendingRef.current = true;
      setPlayerLoading(true);
      setPlayerError(false);
      if (retryBackoffTimeoutRef.current) clearTimeout(retryBackoffTimeoutRef.current);
  ...
  useEffect(() => {
    retryCountRef.current = 0;
    retryPendingRef.current = false;
    if (retryBackoffTimeoutRef.current) {
      clearTimeout(retryBackoffTimeoutRef.current);
      retryBackoffTimeoutRef.current = null;
    }
    setRetryKey(0);
    setPlayerError(false);
    setPlayerLoading(true);
