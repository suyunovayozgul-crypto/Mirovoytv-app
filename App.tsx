...
// source.headers orqali qabul qiladi — VLC'dagi kabi buyruq-qatori
// hiylasi kerak emas, shu bois buildVlcInitOptions olib tashlandi.
...
export default function App() {
  ...
  useEffect(() => {
    // AsyncStorage va pleylist yuklashni PARALLEL boshlaymiz —
    // birinchi yuklash tezlashadi (AsyncStorage kutilmaydi)
    loadEverything();
    (async () => {
      try {
        const [rawFav, rawSettings, rawSync] = await Promise.all([
          AsyncStorage.getItem('mtv:favorites'),
          AsyncStorage.getItem('mtv:settings'),
          AsyncStorage.getItem('mtv:lastSync'),
        ]);
  ...
  if (activeChannel) {
    ...
    return (
      <SafeAreaView style={styles.playerRoot}>
        ...
        {/* ── Video ustida kanal ro'yxati paneli (ekranga bosilganda) ── */}
        <Modal
          visible={controlsVisible && activeChannelList.length > 1}
          transparent
          animationType="slide"
          ...
          <View style={styles.channelListOverlay}>
            <View style={styles.channelListPanel}>
              <View style={styles.channelListHeader}>
                <Text style={styles.channelListTitle}>{activeChannel.group}</Text>
              ...
              <FlatList
                ...
                initialScrollIndex={Math.max(0, activeChannelList.findIndex(c => c.id === activeChannel.id))}
                getItemLayout={(_, index) => ({ length: 56, offset: 56 * index, index })}
                renderItem={({ item }) => {
                  const isActive = item.id === activeChannel.id;
                  return (
                    <TouchableOpacity
                      style={[styles.channelListItem, isActive && styles.channelListItemActive]}
                      onPress={() => {
                        setControlsVisible(false);
                        setPlayerError(false);
                        setPlayerLoading(true);
                    ...
                    >
                      {item.logo ? (
                        ...
                        <View style={[styles.channelListLogoFallback, { backgroundColor: colorFor(item.name) }]}>
                          <Text style={styles.channelListLogoText}>{initials(item.name)}</Text>
                        </View>
                      )}
                      <Text style={[styles.channelListName, isActive && styles.channelListNameActive]} numberOfLines={1}>
                        {item.name}
                      </Text>
                      {isActive && <Ionicons name="play" size={14} color="#8b5cf6" style={{ marginLeft: 'auto' }} />}
                    </TouchableOpacity>
                  );
                }}
              />
...
const styles = StyleSheet.create({
  ...
  // Video ustidagi kanal ro'yxati paneli
  channelListOverlay: {
    flex: 1,
    flexDirection: 'row',
