...
// ═══════════════════════════════════════════════════════════════════════
//  PLEYER: expo-av (ExoPlayer/AVPlayer) O'RNIGA libVLC (ffmpeg asosida)
//
//  Sabab: Android/iOS'ning tayyor pleyeri (ExoPlayer/AVPlayer) faqat
//  standart formatlarni (HLS/m3u8, MP4) yaxshi o'qiydi. Ko'p eski/norasmiy
//  IPTV serverlar xom MPEG-TS oqim yoki g'ayrioddiy kodeklar yuboradi —
//  bularni ExoPlayer ochira olmaydi, lekin VLC (ichida ffmpeg/libavcodec
//  bor) deyarli hammasini o'qiydi. Televizo, IPTV Smarters kabi ilovalar
//  ham aynan shu sabab libVLC'ga tayanadi.
...
// libVLC oddiy "headers" obyektini qabul qilmaydi — buyruq qatori
// uslubidagi initOptions orqali beriladi. Shu bilan birga
// --network-caching qiymatini oshirib qo'yamiz: bu — beqaror IPTV
// serverlarida "tirsillash/to'xtalish"ni sezilarli kamaytiradigan,
// VLC-asosli barcha professional pleyerlarda ishlatiladigan standart
// amaliyot.
function buildVlcInitOptions(url: string): string[] {
  ...
  return [
    ...
    '--clock-jitter=0',
    '--clock-synchro=0',
    // Uzoq muddat (bir necha daqiqadan ortiq) jonli HLS oqimini
    // ko'rishda ba'zi qurilmalarda apparat dekoderi asta-sekin xotira
    // sizdirib, ilovani qulatib qo'yishi (native crash) kuzatilgan —
    // shu sabab dekoderni QATTIQ dasturiy (software) rejimga
    // qulflaymiz.
    '--avcodec-hw=none',
    // Kechikkan kadrlarni bufer to'lib ketmasin deb tashlab yuborish —
    // uzoq ishlashda xotira o'sib ketishining oldini oladi.
    '--drop-late-frames',
    '--skip-frames',
  ];
}
...
export default function App() {
  ...
  // VLCPlayer'da expo-av'dagi kabi videoRef.current.loadAsync() metodi yo'q —
  // qayta yuklash uchun butun komponentni "key" orqali majburiy qayta
  // o'rnatamiz (React'ning standart, ishonchli usuli).
  //
  // MUHIM (haqiqiy ANR bug-reportidan topilgan tuzatish): eski VLCPlayer
  // nusxasi "key" o'zgarib, qayta o'rnatilayotganda (masalan tez-tez kanal
  // almashtirilganda) native tomondan "o'chirilyapman" degan hodisa hali
  // ham kelib qolishi mumkin — bu ESKI, allaqachon almashtirilgan
  // nusxadan kelayotgan "arvoh" chaqiruv. Agar shunga ham qayta urinish
  // bilan javob bersak, cheksiz tsikl hosil bo'lib, protsessorni band
  // qilib, ANR (ilova javob bermay qolishi)ga olib keladi — aynan shu
  // sabab ilova "otib yuborar" edi.
  //
  // Yechim: har bir yuklash urinishiga o'sib boruvchi "avlod" (generation)
  // raqami beriladi. Hodisa faqat "hozirgi avlod" bilan mos kelsagina
  // qabul qilinadi — eskirgan (torn-down) nusxadan kelgan chaqiruvlar
  // e'tiborsiz qoldiriladi.
  const MAX_AUTO_RETRY = 2;
  const LOAD_TIMEOUT_MS = 15000;
  const MIN_RETRY_INTERVAL_MS = 800; // ketma-ket "arvoh" chaqiruvlarga qarshi qo'shimcha himoya
  ...
  const armLoadTimer = useCallback(() => {
    ...
    loadTimeoutRef.current = setTimeout(() => {
      if (loadGenerationRef.current !== genAtSchedule) return; // eskirgan taymer — e'tiborsiz
      attemptRetryOrFail();
  ...
  const attemptRetryOrFail = useCallback(() => {
    ...
    const now = Date.now();
    if (now - lastAttemptAtRef.current < MIN_RETRY_INTERVAL_MS) return; // tez-tez takrorlanishga qarshi
    ...
    if (retryCountRef.current < MAX_AUTO_RETRY) {
      ...
      setLoadGeneration(loadGenerationRef.current); // VLCPlayer'ni majburan qayta o'rnatadi
      armLoadTimer();
    } else {
      clearLoadTimer();
  ...
  }, [activeChannel, armLoadTimer, clearLoadTimer]);

  // Kanal almashganda — hisoblagichni qaytadan boshlaymiz va taymerni
  // o'rnatamiz. MUHIM: bu yerda "loadGeneration" ATAYLAB oshirilmaydi —
  // aks holda VLCPlayer'ning "key"i o'zgarib, har oddiy kanal
  // almashtirishda ham to'liq qayta qurilib ketardi (aynan shu narsa
  // ANR'ga sabab bo'lgani tasdiqlangan). Kanal almashganda faqat
  // `source.uri` yangilanadi — VLCPlayer buni o'zi, qayta qurilmasdan
  // qabul qiladi.
  useEffect(() => {
  ...
  const manualRetry = useCallback(() => {
    retryCountRef.current = 0;
    lastAttemptAtRef.current = 0; // qo'lda bosilganda debounce'ni chetlab o'tamiz
    attemptRetryOrFail();
  }, [attemptRetryOrFail]);
  ...
  // ── EPG (dastur jadvali) — playlist yuklangandan keyin, orqa fonda ──
  const [epg, setEpg] = useState<EpgData | null>(null);
  useEffect(() => {
    fetchEpg().then(setEpg).catch(() => {});
  ...
  if (activeChannel) {
    ...
    return (
      <SafeAreaView style={styles.playerRoot}>
        <StatusBar hidden />
        <VLCPlayer
          // MUHIM: key faqat "avlod (generation)" raqamiga bog'liq,
          // KANAL HAVOLASIGA EMAS. Sabab: ikkita alohida real qurilma
          // bug-reportida tasdiqlandiki, VLCPlayer har safar kanal
          // almashtirilganda TO'LIQ qayta o'rnatilsa, eski native pleyer
          // libvlc_media_player_release() chaqirig'i asosiy oqimni (UI
          // thread) bir necha soniyaga bloklab, Android tomonidan ANR
          // (ilova javob bermayapti) deb majburan yopilishi mumkin. Shu
          // sabab oddiy kanal almashtirishda pleyer qayta qurilmaydi —
          // faqat manba (source) yangilanadi, kutubxonaning o'zi ichki
          // oqimni almashtiradi. To'liq qayta qurish faqat "Qayta
          // urinish" chindan zarur bo'lganda (xato/taymer) sodir bo'ladi.
          key={`vlc-${loadGeneration}`}
          style={styles.video}
          source={{
            uri: activeChannel.url,
            initType: 2,
            // MUHIM: apparat dekoderi (hwDecoder) ba'zi qurilmalarda bir
            // necha daqiqa ishlagandan keyin xotira sizib, ilovani
            // qulatib qo'yishi (native crash) aniqlangan — shu sabab
            // to'liq dasturiy (software) dekoderga o'tkazildi. Biroz
            // ko'proq protsessor ishlatadi, lekin ancha barqaror.
            hwDecoderEnabled: 0,
            hwDecoderForced: 0,
          ...
          onError={() => {
            // Bu nusxa "hozirgi avlod"ga tegishlimi — tekshiramiz.
            // Aks holda, eskirgan (qayta o'rnatilayotgan) nusxadan kelgan
            // "arvoh" chaqiruv bo'lishi mumkin — buni e'tiborsiz qoldirish
            // shart, aks holda cheksiz qayta urinish tsikli va ANR
            // (ilova javob bermay qolishi) yuzaga kelishi mumkin.
            if (loadGenerationRef.current !== loadGeneration) return;
            attemptRetryOrFail();
          }}
          // MUHIM: onStopped ATAYLAB ULANMAGAN. Bu hodisa VLCPlayer
          // O'ZI TEGISHLI TARZDA (masalan kanal almashtirilganda, eski
          // nusxa qayta o'rnatilayotganda) yopilganda ham ishga tushadi —
          // haqiqiy xato belgisi emas. Buni ham qayta-urinish sifatida
          // talqin qilish, real qurilmada ANR (ilova muzlab qolishi)ga
          // olib kelgani aniqlangan (bug-report orqali tasdiqlangan).
        />
