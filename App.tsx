...
function buildVlcInitOptions(url: string): string[] {
  ...
  return [
    ...
    // qulflaymiz (hwDecoderEnabled/Forced prop'lariga qo'shimcha,
    // ba'zi VLC versiyalarida faqat shu bayroq orqaligina to'liq
    // o'chadi).
    '--avcodec-hw=none',
  ...
  if (activeChannel) {
    ...
    return (
      <SafeAreaView style={styles.playerRoot}>
        <StatusBar hidden />
        <VLCPlayer
          // MUHIM: key faqat "retryKey"ga bog'liq, kanal havolasiga EMAS.
          // Sabab: bug-report tahlili shuni ko'rsatdiki, VLCPlayer har safar
          // kanal almashtirilganda (key o'zgarganda) TO'LIQ qayta o'rnatilib,
          // eski native pleyer libvlc_media_player_release() orqali
          // yopiladi — bu chaqiruv esa asosiy oqimni (UI thread) BLOKLAB
          // qo'yishi (ba'zan 5+ soniyaga) va Android tomonidan ANR (ilova
          // javob bermayapti) deb MAJBURAN yopilishi tasdiqlandi (bir xil
          // "vlc_join → libvlc_media_player_release" izi ikkita alohida
          // bug-report'da chiqdi). Shu sabab endi oddiy kanal almashtirishda
          // pleyer QAYTA O'RNATILMAYDI — faqat manba (source) yangilanadi,
          // kutubxonaning o'zi ichki oqimni almashtiradi. Faqat "Qayta
          // urinish" bosilganda (retryKey o'zgarganda) to'liq qayta
          // o'rnatiladi — bu kamdan-kam va foydalanuvchi kutmoqda.
          key={`vlc-${retryKey}`}
          style={styles.video}
          source={{
