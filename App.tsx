...
export default function App() {
  ...
  // ── VLCPlayer mount/unmount race tuzatish ──────────────────────────
  // MUAMMO: kanal almashtirilganda VLCPlayer "key" orqali bir zumda
  // unmount+mount bo'ladi. Lekin native libVLC obyekti ASINXRON tarzda
  // tozalanadi — u hali to'liq o'chib ulgurmasdan, Android Activity
  // "onPause" chaqirsa (ekran o'chishi, tez-tez kanal almashtirish,
  // fon rejimga o'tish), kutubxona allaqachon yo'q qilinayotgan native
  // obyektga pause() chaqiradi va ilova
  // "IllegalStateException: can't get VLCObject instance" bilan qulaydi
  // (ReactVlcPlayerView.onHostPause). Buni bug report'dan tasdiqladik.
  //
  // YECHIM: eski pleyerni AVVAL butunlay unmount qilamiz (vlcMounted=false),
  // native tomonda tozalanishga vaqt beramiz (kichik delay), SO'NGRA
  // yangi pleyerni mount qilamiz. Shu bilan bir vaqtning o'zida ikkita
  // VLCPlayer instance yashamaydi va eski, "o'lik" obyektga chaqiruv
  // ketmaydi.
  const [vlcMounted, setVlcMounted] = useState(true);
  const vlcRemountTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    if (!activeChannel) return;
    setVlcMounted(false); // eski pleyerni darhol unmount qilamiz
    if (vlcRemountTimerRef.current) clearTimeout(vlcRemountTimerRef.current);
    vlcRemountTimerRef.current = setTimeout(() => {
      setVlcMounted(true); // native tomon tozalangач, yangisini mount qilamiz
    }, 220);
    return () => {
      if (vlcRemountTimerRef.current) clearTimeout(vlcRemountTimerRef.current);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeChannel?.url, retryKey]);

  // ── Ilova fon rejimiga o'tganda pleyerni oldindan unmount qilamiz ──
  // Shu bilan Android "onPause" eski/"o'lik" native obyektga emas,
  // umuman mavjud bo'lmagan komponentga tegadi — xavfsizroq.
  useEffect(() => {
    const sub = AppState.addEventListener('change', (nextState) => {
      if (nextState !== 'active') {
        ...
        if (vlcRemountTimerRef.current) clearTimeout(vlcRemountTimerRef.current);
        vlcRemountTimerRef.current = setTimeout(() => setVlcMounted(true), 220);
      }
    });
    return () => sub.remove();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const attemptRetryOrFail = useCallback(() => {
    ...
    if (retryCountRef.current < MAX_AUTO_RETRY) {
      ...
      setRetryKey((k) => k + 1); // VLCPlayer'ni majburan qayta o'rnatib, oqimni qaytadan ochadi
      armLoadTimer();
    } else {
      clearLoadTimer();
  ...
  // Kanal almashganda — hisoblagichni qaytadan boshlaymiz va taymerni o'rnatamiz
  useEffect(() => {
        ...
        {vlcMounted && (
          <VLCPlayer
            ...
            source={{
              ...
              // necha daqiqa ishlagandan keyin xotira sizib, ilovani qulatib
              // qo'yishi aniqlandi — shu sabab dasturiy (software) dekoderga
              // o'tkazildi (biroz ko'proq protsessor ishlatadi, lekin ancha
              // barqaror, uzoq muddat qulamaydi).
              hwDecoderEnabled: 0,
              hwDecoderForced: 0,
            ...
            onStopped={() => {
              // Server oqimni kutilmaganda uzib qo'yishi mumkin — bu ham xato
              // sifatida ko'rilib, qayta ulanish zanjiriga kiritiladi.
              attemptRetryOrFail();
            }}
          />
        )}
...
const styles = StyleSheet.create({
  ...
  // MUHIM: backgroundColor QASDDAN OLIB TASHLANDI — VLCPlayer ichida
  // ishlatiladigan Android TextureView fon rangini qo'llab-quvvatlamaydi
  // va uni berishga urinish ilovani "TextureView doesn't support
  // displaying a background drawable" xatoligi bilan qulatib yuboradi
  // (JSApplicationIllegalArgumentException, RCTVLCPlayer).
  // Fon qorayishi kerak bo'lsa, buning o'rniga uni o'rab turgan
  // playerRoot (SafeAreaView) da beriladi — u oddiy View, muammosiz.
  video: { flex: 1 },
