...
export async function fetchEpg(overrideUrl?: string): Promise<EpgData> {
  const url = overrideUrl && overrideUrl.trim() ? overrideUrl.trim() : EPG_URL;
  ...
  try {
    ...
    // aks holda EPG butunlay bo'sh qaytishi mumkin. Referer manbaning
    // o'z domenidan avtomatik olinadi.
